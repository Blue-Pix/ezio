...
mod dhcp;
mod util;
use dhcp::{DhcpPacket, DhcpServer, OPTIONS};
...
pub fn run() {
  ...
  let dhcp_server = Arc::new(DhcpServer::new().unwrap_or_else(|e| {
...
fn dhcp_handler(
  ...
  let message = packet.get_option(Code::MessageType as u8).ok_or_else(|| {
    failure::err_msg("specified options was not found")
  })?;
  ...
  match message_type {
    ...
    DHCPRELEASE => dhcp_release_message_handler(transaction_id, dhcp_server, &packet, client_macaddr),
    _ => {
...
fn select_lease_ip(dhcp_server: &Arc<DhcpServer>, received_packet: &DhcpPacket) -> Result<Ipv4Addr, failure::Error> {
  {
    ...
    while let Some(ip_addr) = dhcp_server.pick_avaiable_ip() {
...
fn obtain_available_ip_from_requested_option(
  ...
  let ip = received_packet.get_option(Code::RequestIpAddress as u8)?;
...
fn make_dhcp_packet(
  ...
  dhcp_packet.set_option(&mut cursor, Code::ServerIdentifier as u8, 4, Some(&dhcp_server.server_addr.octets()));
...
fn dhcp_request_message_handler_responded_to_offer(
  ...
  let server_ip = util::u8_to_ipv4addr(&server_id).ok_or_else(|| {
    failure::err_msg("Failed to convert ip addr.")
  })?;

  if server_ip != dhcp_server.server_addr {
    info!("Client has chosen another dhcp server.");
    return Ok(())
  }

  let ip_bin = received_packet.get_option(Code::RequestIpAddress as u8).unwrap();
  let ip_to_be_leased = util::u8_to_ipv4addr(&ip_bin).ok_or_else(|| {
    failure::err_msg("Failed to convert ip addr.")
  })?;
...
fn dhcp_request_message_handler_to_reallocate(
  ...
  if let Some(requested_ip) = received_packet.get_option(Code::RequestIpAddress as u8) {
    debug!("client is in INIT_REBOOT");
    let requested_ip = util::u8_to_ipv4addr(&requested_ip).ok_or_else(|| {
      failure::err_msg("Failed to convert ip addr.")
    })?;
