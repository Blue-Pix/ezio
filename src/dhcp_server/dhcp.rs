...
impl DhcpServer {
  pub fn new() -> Result<DhcpServer, failure::Error> {
    ...
    Ok(DhcpServer{
      ...
      server_addr: static_addresses["dhcp_server_addr"],
  ...
  fn init_address_pool(
    ...
    let mut addr_pool: Vec<Ipv4Addr> = network_addr_with_prefix.iter().filter(|addr| {
      !used_ip_addrs.contains(addr)
    }).collect();
    addr_pool.reverse();
    Ok(addr_pool)
  }
  ...
  pub fn pick_avaiable_ip(&self) -> Option<Ipv4Addr> {
    let mut lock = self.address_pool.write().unwrap();
    lock.pop()
  }
...
impl DhcpPacket {
  pub fn new(buffer: Vec<u8>) -> Option<DhcpPacket> {
    if buffer.len() > DHCP_MINIMUM_SIZE {
  ...
  pub fn get_chaddr(&self) -> MacAddr {
    let b = &self.buffer[CHADDR..CHADDR+6];
  ...
  pub fn get_option(&self, options_code: u8) -> Option<Vec<u8>> {
    let mut index: usize = 4;
    let options = self.get_options();
    while options[index] != OPTION_END {
      if options[index] == options_code {
