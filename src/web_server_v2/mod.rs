...
use std::str;
use regex::Regex;
use mio::{Poll, Token, Event, Events, Ready, PollOpt};
    ...
    loop {
      ...
      for event in &events {
        match event.token() {
          ...
          Token(conn_id) => {
            self.http_handler(conn_id, event, &poll, &mut response).unwrap_or_else(|e| {
              error!("{}", e);
            })
          }
        }
...
fn create_msg_from_code(status_code: u16, msg: Option<Vec<u8>>) -> Result<Vec<u8>, failure::Error> {
  match status_code {
    ...
    _ => Err(failure::err_msg("No such a status code implemeted.")),
  }
}
